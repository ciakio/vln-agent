...
class ApproachAndNavigate:
    ...
    def run(self) -> bool:
        ...
        if rospy is not None:
            rospy.loginfo("[approach] === 正面逼近开始: target=%s, camera=%s, scene=%s ===",
        ...
        if rgb_key not in images or depth_key not in images:
            ...
            return False
        if rospy is not None:
            rospy.loginfo("[approach] 采集成功: rgb=%s", list(images[rgb_key].shape))

        # 2. VLM 识别（第 1 次视觉检测：初始定位）
        if rospy is not None:
            rospy.loginfo("[approach] 第 1 次视觉检测（初始定位）→ VLM")
        ...
        if not result.success:
            ...
            return False
        if rospy is not None:
            rospy.loginfo("[approach] VLM 定位成功 (%.0fms): 目标深度=%.3fm, 支撑面=%.3fm, "
        ...
        if rospy is not None:
            rospy.loginfo("[approach] 计算导航目标点: (%.3f, %.3f, %.1f°), 当前 (%.3f, %.3f)",
        ...
        if rospy is not None:
            rospy.loginfo("[approach] 等待导航到达 (超时 30s)...")
        t0 = time.time()
        arrived = False
        ...
        if rospy is not None:
            rospy.loginfo("[approach] 导航等待结束: arrived=%s, 耗时 %.1fs",
        ...
        if rospy is not None:
            rospy.loginfo("[approach] 到位, 等待 1s 稳定后二次验证...")
        self._sync_pose()
        if rospy is not None:
            rospy.sleep(1.0)
        else:
        ...
        if rospy is not None:
            rospy.loginfo("[approach] 第 2 次视觉检测（到位后复核）→ VLM")
        verify_result = self.vlm.compute(
            images[rgb_key], images[depth_key], intrinsics, self.camera)
        self.verify_result = verify_result
        if rospy is not None and verify_result.success:
            rospy.loginfo("[approach] 二次验证成功: 支撑面深度=%.3fm, conf=%.2f",
