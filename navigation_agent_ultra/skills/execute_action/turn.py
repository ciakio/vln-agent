...
底层完全复用 common.motion.MotionController.rotate_to（真机调好的 PID、
...
if __package__:
    ...
    from ..common.ros_utils import normalize_angle
else:
    import os as _os
    sys.path.insert(0, _os.path.abspath(_os.path.join(_os.path.dirname(__file__), '..', '..')))
    from skills.common.motion import MotionController
    from skills.common.ros_utils import normalize_angle
...
class Turn:
    ...
    def run(self):
        ...
        if rospy is not None:
            rospy.loginfo("[turn] 模式=%s, 起始朝向=%.2f°, 请求=%.2f°, 目标绝对角=%.2f°",
                          self.mode, self.start_theta_deg, self.requested, self.target_yaw)

        # 复用公共 PID 旋转(绝对角度); timeout 固定使用 config.ROTATE_TIMEOUT, 不暴露参数
        ...
        if rospy is not None:
            rospy.loginfo("[turn] 结束: ok=%s, mode=%s, 最终朝向=%.2f°, 残差=%.2f°",
...
if __name__ == "__main__":
    ...
    if rospy is not None and not rospy.core.is_initialized():
        rospy.init_node("turn_cli", anonymous=True)
    node = Turn(yaw=args.yaw, delta_yaw=args.delta_yaw)
    print(node.run())
