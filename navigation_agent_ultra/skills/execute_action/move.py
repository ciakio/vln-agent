...
导航栈，轮询 odom 判到位，到位/超时后 cancel + 双零速截停。

不调用相机、不做 VLM 搜索，纯位移；走导航栈（会避障、依赖全局地图）。
...
class Mover:
    ...
    def run(self):
        ...
        # 1) 需要时先转到前进朝向（复用固定 PID 旋转）
        if self.turn_mode != "none":
            if rospy is not None:
                rospy.loginfo("[move] 先转到前进朝向 %.1f° (模式=%s)",
        ...
        if rospy is not None:
            rospy.loginfo("[move] 沿 %.1f° 前进 %.2fm: 起点(%.3f, %.3f) → 目标(%.3f, %.3f), 超时 %.0fs",
        ...
        while not (rospy is not None and rospy.is_shutdown()):
            ...
            if time.time() - t0 > move_timeout:
                if rospy is not None:
                    rospy.logwarn("[move] 前进超时(%.0fs), 当前距目标点 %.3fm", move_timeout, dist)
                break
            time.sleep(0.1)

        # 3) 覆盖目标强行截停底层导航 + 双零速 + 物理缓冲（与 GoTurn 一致）
        try:
            self.motion.cancel_navigation()
        except Exception as e:
            if rospy is not None:
                rospy.logwarn("[move] 覆盖目标失败(可忽略): %s", str(e))
        ...
        if rospy is not None:
            rospy.loginfo("[move] 结束: ok=%s, 请求=%.2fm, 实际=%.3fm, 残差=%.3fm, 耗时 %.1fs",
...
if __name__ == "__main__":
    ...
    if rospy is not None and not rospy.core.is_initialized():
        rospy.init_node("move_cli", anonymous=True)
    node = Mover(args.distance, yaw=args.yaw, delta_yaw=args.delta_yaw)
    print(node.run())
