...
class Explorer:
    ...
    def __init__(self, target_name="白色桌子上的白绿色的椰子水饮料瓶", out_dir="explore_logs",
        ...
        if rospy is not None:
            rospy.loginfo("[explore] 初始化: target=%s, max_rounds=%d, camera=%s, 起始位姿(%.3f,%.3f,%.1f°)",
    ...
    def move_to(self, target_x, target_y):
        ...
        if rospy is not None:
            rospy.loginfo("[explore.move_to] 目标(%.3f,%.3f), 初始距离 %.3fm",
        ...
        if rospy is not None:
            rospy.loginfo("[explore.move_to] 结束: 剩余 %.3fm, 到达=%s, 实际位姿(%.3f,%.3f)",
    ...
    def scan_round(self, round_i, angles_to_scan):
        ...
        def vlm_worker(view_i, theta_deg, robot_x, robot_y, img, img_paths):
            ...
            if rospy is not None:
                ...
                rospy.loginfo("[explore.scan] r%d 方位 %.1f°: found=%s, conf=%s, open=%.2f, 物体%d个, %s",
            ...
            with lock:
                ...
                if analysis.get("found") and found_data["entry"] is None:
                    if rospy is not None:
                        rospy.loginfo("[explore.scan] *** 在 %.1f° 方位发现目标! 位姿(%.3f,%.3f) ***",
        ...
        for view_i, target_theta in enumerate(angles_to_scan):
            ...
            self.view_i = view_i
            if rospy is not None:
                rospy.loginfo("[explore.scan] r%d 扫描方位 %d/%d -> %.0f°",
            ...
            if self.camera not in imgs:
                if rospy is not None:
                    rospy.logwarn("[explore.scan] %.0f° 图像采集失败, 记为 None", target_theta)
                results.append(None)
                continue
        ...
        if found_event.is_set() and found_data["pose"] is not None:
            ...
            if rospy is not None:
                rospy.loginfo("[explore.scan] 本轮发现目标, 旋转回发现方位 %.1f°", theta_found)
    ...
    def backtrack(self):
        ...
        if len(self.waypoints) <= 1:
            if rospy is not None:
                rospy.logwarn("[explore] waypoint 栈已空, 无法回退")
            return False
        ...
        if rospy is not None:
            rospy.loginfo("[explore] 回退到上一 waypoint: (%.3f, %.3f), 朝向 %.1f°",
                          tx, ty, bearing)
    ...
    def run(self):
        if rospy is not None and not rospy.core.is_initialized():
            rospy.init_node("navigate_by_goal_node", anonymous=True)
        ...
        try:
            ...
            while round_i < self.max_rounds and not (rospy is not None and rospy.is_shutdown()):
                if rospy is not None:
                    rospy.loginfo("[explore] ===== 第 %d/%d 轮探索 =====",
                ...
                if rospy is not None:
                    rospy.loginfo("[explore] 本轮扫描方位: %s", angles_to_scan)
                ...
                if not valid:
                    fail_rounds += 1
                    if rospy is not None:
                        rospy.logwarn("[explore] 本轮无有效采集 (连续 %d/%d 轮)",
                ...
                if chosen is None:
                    if rospy is not None:
                        rospy.logwarn("[explore] 无可用开阔方向, 尝试回退到上一 waypoint")
                    ...
                    if rospy is not None:
                        rospy.loginfo("[explore] 沿选定方向 %.1f° 前进 %.1fm (score=%.2f)",
    ...
    def _finalize(self, status):
        self.status = status
        if rospy is not None:
            rospy.loginfo("[explore] 探索结束: status=%s, 共 %d 条视野记忆, %d 个语义物体, waypoint %d 个",
