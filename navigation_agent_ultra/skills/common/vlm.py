...
class VLMApproach:
    ...
    def __init__(self, target_description: str, scene_type: str = "shelf"):
        ...
        if not VLM_API_KEY:
            try:
                import rospy
                rospy.logfatal("KSC_API_KEY 或 VLM_API_KEY 未设置, 请运行: export KSC_API_KEY=<你的key>")
            except ImportError:
                print("KSC_API_KEY 或 VLM_API_KEY 未设置, 请运行: export KSC_API_KEY=<你的key>")
            raise RuntimeError("KSC_API_KEY 或 VLM_API_KEY 未设置, 请运行: export KSC_API_KEY=<你的key>")
        self.client = OpenAI(
            api_key=VLM_API_KEY,
    ...
    def compute(self, bgr, depth, intrinsics, camera_name: str) -> ApproachResult:
        ...
        if parsed is None:
            try:
                import rospy
                rospy.logerr(f"VLM 调用失败（重试后）: {last_error}")
            except ImportError:
