...
class VLMApproach:
    ...
    def compute(self, bgr, depth, intrinsics, camera_name: str) -> ApproachResult:
        ...
        """
        from .ros_utils import bgr_to_base64_uri, depth_sample_bbox
        ...
        if rospy is not None:
            rospy.loginfo("[VLM.approach#%d] 开始检测: target=%s, camera=%s, 图像=%dx%d",
                          seq, self.target_description, camera_name, bgr.shape[1], bgr.shape[0])

        if not self.check_auth():
            if rospy is not None:
                rospy.logerr("[VLM.approach#%d] 鉴权失败: API Key 无效或网络不可达", seq)
        ...
        for attempt in range(2):
            try:
                if rospy is not None and attempt == 1:
                    rospy.logwarn("[VLM.approach#%d] 首次调用/解析失败, 进行第 2 次尝试", seq)
                ...
                try:
                    ...
                    pass
                if rospy is not None:
                    rospy.loginfo("[VLM.approach#%d] 第%d次尝试 原始返回:\n%s",
                ...
                last_error = e
                if rospy is not None:
                    rospy.logwarn("[VLM.approach#%d] 第 %d 次调用异常: %s", seq, attempt + 1, e)
                if attempt == 0:
                    time.sleep(1.0)
                    continue
                break
        ...
        if parsed is None:
            if rospy is not None:
                rospy.logerr("[VLM.approach#%d] VLM 调用失败（重试后）: %s", seq, last_error)
            else:
                print(f"VLM 调用失败（重试后）: {last_error}")
        ...
        if parsed.get("_parse_failed"):
            ...
            if rospy is not None:
                rospy.logerr("[VLM.approach#%d] JSON 解析失败(重试后): %s", seq, last_error)
        ...
        if not parsed.get("found", False):
            ...
            result.latency_ms = (time.time() - t0) * 1000
            if rospy is not None:
                rospy.loginfo("[VLM.approach#%d] 未找到目标 (%.0fms): %s",
        ...
        try:
            ...
            if object_bbox is None:
                result.success = False
                if rospy is not None:
                    rospy.logwarn("[VLM.approach#%d] found=true 但包围盒无效: %s",
        ...
        try:
            ...
            if Z_target <= 0.0:
                result.success = False
                if rospy is not None:
                    rospy.logwarn("[VLM.approach#%d] 目标包围盒 %s 内无有效深度", seq, object_bbox)
            ...
            result.latency_ms = (time.time() - t0) * 1000
            if rospy is not None:
                rospy.loginfo("[VLM.approach#%d] 检测成功 (%.0fms): conf=%.2f, 目标深度=%.3fm, "
            ...
            result.success = False
            if rospy is not None:
                rospy.logerr("[VLM.approach#%d] 结果解析异常: %s", seq, e)
...
class VLMSceneAnalyzer:
    ...
    def analyze(self, system_prompt: str, user_prompt: str,
        ...
        t0 = time.time()
        if rospy is not None:
            rospy.loginfo("[VLM.scene#%d] 开始场景分析: 图像 %d 张, max_tokens=%d, temp=%.2f",
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    ...
                    pass
                if rospy is not None:
                    rospy.loginfo("[VLM.scene#%d] 第%d次尝试 原始返回:\n%s",
                                  seq, attempt + 1, _log_truncate(raw))
                ...
                if parsed is not None:
                    if rospy is not None:
                        ...
                        rospy.loginfo("[VLM.scene#%d] 分析完成 (%.0fms): found=%s, conf=%s, "
                                      "open=%.2f, 物体 %d 个, 环境=%s",
                ...
                # JSON 解析失败，重试一次
                if rospy is not None:
                    rospy.logwarn("[VLM.scene#%d] JSON 解析失败, 第 %d 次", seq, attempt + 1)
                if attempt == 0:
                    time.sleep(1.0)
                    continue
                if rospy is not None:
                    rospy.logerr("[VLM.scene#%d] 重试后 JSON 仍解析失败", seq)
                return None
            except Exception as e:
                if rospy is not None:
                    rospy.logwarn("[VLM.scene#%d] 第 %d 次调用异常: %s", seq, attempt + 1, e)
                if attempt == 0:
                    time.sleep(1.0)
                    continue
                if rospy is not None:
                    rospy.logerr("[VLM.scene#%d] 重试后仍调用失败: %s", seq, e)
                return None
