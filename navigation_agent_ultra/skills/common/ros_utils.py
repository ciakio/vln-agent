...
class OdomListener:
    ...
    def __init__(self, topic: str = ODOM_TOPIC):
        ...
        rospy.loginfo("[OdomListener] 已订阅里程计话题: %s", topic)
    ...
    def wait_for_odom(self, timeout: float = ODOM_TIMEOUT) -> bool:
        ...
        while not rospy.is_shutdown():
            with self._lock:
                if self._received:
                    rospy.loginfo("[OdomListener] 收到首帧 odom: (%.3f, %.3f, %.3frad)",
                                  self._x, self._y, self._theta)
                    return True
            if rospy.Time.now().to_sec() - t0 > timeout:
                rospy.logwarn("[OdomListener] 等待首帧 odom 超时(%.1fs), topic=%s",
                              timeout, self.topic)
                return False
            rospy.sleep(0.05)
        return False
...
class MultiImageSaver:
    ...
    def __init__(self, topics):
        ...
        if rospy is not None:
            rospy.loginfo("[MultiImageSaver] 开始采集 %d 路图像: %s",
    ...
    def get_images(self, timeout_sec=CAPTURE_TIMEOUT):
        ...
        self.unregister()
        if len(self.got) >= len(self.topics):
            shapes = {k: list(self.data[k].shape) for k in self.data}
            rospy.loginfo("[MultiImageSaver] 采集成功: %s, 耗时 %.2fs",
                          shapes, rospy.Time.now().to_sec() - t0)
        else:
            rospy.logwarn("[MultiImageSaver] 采集超时(%.1fs), 仅收到 %s, 需要 %s",
                          timeout_sec, list(self.got), list(self.topics.keys()))
...
class SingleImageSaver:
    ...
    def __init__(self, topic):
        ...
        self.topic = topic
        if rospy is not None:
            rospy.loginfo("[SingleImageSaver] 开始采集单路图像: %s", topic)
    ...
    def get_image(self, timeout_sec=CAPTURE_TIMEOUT):
        ...
        while not rospy.is_shutdown():
            if self.got:
                self.unregister()
                rospy.loginfo("[SingleImageSaver] 采集成功: %s, shape=%s, 耗时 %.2fs",
                              self.topic, list(self.data.shape),
                              rospy.Time.now().to_sec() - t0)
                return self.data
            if rospy.Time.now().to_sec() - t0 > timeout_sec:
                break
            rospy.sleep(0.05)
        self.unregister()
        rospy.logwarn("[SingleImageSaver] 采集超时(%.1fs): %s", timeout_sec, self.topic)
        return None
...
def capture_rgb_depth(camera: str = "chest") -> dict:
    ...
    if rospy is not None:
        rospy.loginfo("[capture_rgb_depth] 相机=%s, rgb=%s, depth=%s",
                      camera, rgb_topic, depth_topic)
    ...
    if images and f"{camera}_rgb" in images and f"{camera}_depth" in images:
        return images
    if rospy is not None:
        rospy.logwarn("[capture_rgb_depth] 相机=%s RGB+深度采集不完整, 返回空", camera)
    return {}
...
def capture_rgb(camera: str = "chest"):
    ...
    if rospy is not None:
        rospy.loginfo("[capture_rgb] 相机=%s, topic=%s", camera, topic)
...
def project_to_global(robot_x, robot_y, theta_deg, depth, bearing_delta_deg=0.0):
    ...
    Args:
        robot_x, robot_y: 机器人当前全局坐标（米，与 odom / navigate_to_point 同系）
        theta_deg: 机器人当前全局朝向（度）
        depth: 目标相对机器人的距离（米，>0），通常取 VLM 目标框中位深度
        bearing_delta_deg: 目标相对机器人朝向的水平方位角（度，正=左/逆时针）；
            已对正（目标在视野中央）时传 0。
    Returns:
        (global_x, global_y)，保留 3 位小数；depth 缺失/非正返回 (None, None)。
    纯函数、不依赖 ROS，可在无 ROS 的 Windows 直接单测。
    """
