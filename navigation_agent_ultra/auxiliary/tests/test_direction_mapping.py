...
from skills.navigation.navigate_by_goal import (
    DIRECTION_OFFSET as EXPLORE_DIR_OFFSET,
    DISTANCE_METERS as EXPLORE_DIST,
)
from skills.vision_observe.look_around import (
...
def test_explore_direction_mapping():
    """navigate_by_goal 的 8 方位偏移符合 ROS 坐标系。"""
    ...
    print("[PASS] navigate_by_goal 方向映射符合 ROS 约定")


def test_observe_direction_mapping():
    """look_around 的 8 方位偏移与 navigate_by_goal 一致。"""
    assert OBSERVE_DIR_OFFSET == EXPLORE_DIR_OFFSET, \
        "look_around 与 navigate_by_goal 的 DIRECTION_OFFSET 必须一致"
    print("[PASS] look_around 方向映射与 navigate_by_goal 一致")


def test_observe_direction_names():
    """look_around 的 DIRECTION_NAMES 标签与角度对应正确。"""
...
def _project(robot_x, robot_y, robot_theta_deg, direction, distance_level,
    ...
    from skills.navigation.navigate_by_goal import HORIZONTAL_OFFSET
...
# ===================================================================
# 4. navigate_by_goal 方位筛选：排除开阔度最高的 N 个方向
# ===================================================================
