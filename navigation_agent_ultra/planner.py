...
- navigate_by_route: 沿当前方向【离散步进】前进：尚未看到目标时每步走4m大步搜索；一旦VLM锁定目标，就按“上一停车帧测得的目标深度”自适应缩短步长（深度≥6m走5m、4~6m走3m、2~4m走1m，越近越慢），每步停车VLM找物。只有目标深度落在 0~2m 成功线内才停止（成功线由系统硬固定，你传的 stop_depth/step_dist 不再决定成败、可省略），停稳后原地旋转到 angle 指定朝向（没有转向需求就填当前朝向、保持原方向）。动作起步时会先看一帧，若目标已在0~2m则直接到位、不盲走；若某一步底盘实际没动会判未推进、连续两次即失败交你换招。route 到位即【完成】，不做0.5m贴近——精准逼近(approach)是 navigate_by_goal 粗搜路线的收尾，route 已有既定路径、到位即止，其后【不接】 approach_aligned/diagonal（除非该段 done_criteria 明确要求贴到跟前）。
...
- turn: 原地旋转到指定朝向，不平移。
  ...
  前置条件: 无（只依赖底盘 odom 闭环，不依赖全局地图，也不调用VLM）。
...
- move: 沿指定方向前进“确定距离”，纯位移，不调用相机、不做视觉搜索、不消耗VLM（走导航栈，会避障、依赖全局地图）。
  ...
  前置条件: 全局地图/导航栈可用；只支持前进，不支持后退（后退请先 turn(delta_yaw=180) 掉头再 move）。
  参数:
...
class Planner:
    ...
    def extract_keywords(self, instruction):
        ...
        if rospy is not None:
            rospy.loginfo("[Planner] 关键词提取: %s", instruction)
        ...
        if result is None:
            if rospy is not None:
                rospy.logwarn("[Planner] 关键词 LLM 无返回, 降级为规则匹配")
            # 降级: 简单规则提取
        ...
        if rospy is not None:
            rospy.loginfo("[Planner] 关键词提取结果: targets=%s, actions=%s, constraints=%s",
    ...
    def plan_baseline(self, instruction, memory_context="", current_pose=None):
        ...
        """
        if rospy is not None:
            rospy.loginfo("[Planner] 行动基准规划开始 (LLM), 当前位姿: %s",
        ...
        for i, seg in enumerate(baseline):
            ...
            for forbidden in ("params", "resolve", "resolve_hint"):
                if forbidden in seg:
                    ...
                    if rospy is not None:
                        rospy.logwarn("[Planner] 基准段 %d 误填 %s 已剔除（基准不可执行）",
                                      i + 1, forbidden)
            seg.setdefault("goal", "")
        ...
        if rospy is not None:
            rospy.loginfo("[Planner] 行动基准规划完成: %d 段, 理解=%s",
    ...
    def plan_next_step(self, instruction, baseline_view, progress_view,
        ...
        """
        if rospy is not None:
            rospy.loginfo("[Planner] 在线单步决策开始, 当前位姿: %s",
                          self._format_pose(current_pose))
        ...
        for attempt in range(self.STEP_DECISION_MAX_RETRY + 1):
            ...
            if attempt > 0:
                if rospy is not None:
                    rospy.logwarn("[Planner] 在线决策第 %d 次修正重试（上次错误: %s）",
                                  attempt, last_err)
                cur_messages = messages + [
        ...
        if decision.get("task_done"):
            if rospy is not None:
                rospy.loginfo("[Planner] 在线决策: 任务完成, last_review=%s",
                              decision.get("last_review", ""))
            return decision

        if rospy is not None:
            rospy.loginfo("[Planner] 在线单步决策: 段%s(phase=%s) %s.%s %s, 期望核对=%s, 依据=%s",
