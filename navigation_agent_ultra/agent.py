...
# —— 动作有效性闸门（识别“指令发了但机器人几乎没动”，真机底层运动稳定时主要防指令未执行）——
...
class NavigationAgent:
    ...
    def _install_signal_handlers(self):
        ...
        保证本机 import/py_compile 不受影响；真机 Linux 下四类信号均可注册。
        """
        for sig_name in ("SIGINT", "SIGTERM", "SIGTSTP", "SIGHUP"):
            ...
            try:
                ...
                if rospy is not None:
                    rospy.logwarn("[NavigationAgent] 信号 %s 无法注册(可忽略): %s", sig_name, e)
    ...
    def _emergency_persist(self, signum=None, frame=None):
        ...
        try:
            if rospy is not None:
                rospy.logerr("[NavigationAgent] 收到中断信号 %s，紧急落盘后直接退出...", sig_name)
        except Exception:
            print("[NavigationAgent] 收到中断信号 %s，紧急落盘后直接退出..." % sig_name, flush=True)
    ...
    def _seg_completion_gate(self, seg, action, params, result, pre_pose, post_pose):
        ...
        if seg.get("terminal") and vg == "pass":
            if rospy is not None:
                rospy.logwarn("[NavigationAgent] 段%s 为终点段，visual_gate=pass 被钳制为 hold",
                              seg.get("seg"))
            vg = "hold"
        ...
        if rospy is not None:
            rospy.loginfo("[NavigationAgent] 段%s 联合闸门: vg=%s, 运动=%s(%s), 视觉=%s(%s)",
    ...
    def _perceive_before_decision(self, instruction, baseline_view, baseline, cursor,
        ...
        if mode == "skip":
            if rospy is not None:
                rospy.loginfo("[视觉前哨] 段%d 首选 %s 为纯几何转向，本轮跳过 VLM",
        ...
        try:
            ...
            for fi in range(n_frames):
                ...
                if rospy is not None:
                    ...
                    rospy.loginfo("[视觉前哨] 段%d 模式=%s 第%d/%d帧返回: %.600s",
            ...
            if rospy is not None:
                rospy.loginfo("[视觉前哨] 段%d 模式=%s 决策前感知文本:\n%s", cursor, mode, view)
    ...
    def _glance_to_memory(self, parsed, geom, current_pose, seg):
        ...
        try:
            ...
            if rospy is not None:
                rospy.logwarn("[视觉前哨] 瞥见写记忆失败(可忽略): %s", e)
    ...
    def run(self, instruction):
        ...
        rospy.loginfo("[NavigationAgent] [3/6] 行动基准规划...")
        ...
        rospy.loginfo("[NavigationAgent] 行动基准共 %d 段:", len(baseline))
        for seg in baseline:
            tags = seg.get("step_type", "")
            if seg.get("terminal"):
                tags = (tags + "/终点").strip("/")
            rospy.loginfo("[NavigationAgent]   段%d %s.%s[%s] — %s%s",
                          seg.get("seg", 0), seg.get("skill", ""), seg.get("action", ""),
                          tags, seg.get("goal", ""),
                          f"（备注: {seg['note']}）" if seg.get("note") else "")
            if seg.get("done_criteria"):
                rospy.loginfo("[NavigationAgent]       完成判据: %s", seg["done_criteria"])
            if seg.get("expect_to_see"):
                rospy.loginfo("[NavigationAgent]       预期所见: %s",
        ...
        # 步骤 4: 在线逐步执行
        rospy.loginfo("[NavigationAgent] [4/6] 在线逐步执行...")
        ...
        rospy.loginfo("[NavigationAgent] Review 结果: success=%s, 完成段 %d/%d, "
    ...
    def _run_online_loop(self, instruction, baseline_plan, memory_context):
        ...
        rospy.loginfo("[NavigationAgent] 在线闭环启动: 基准 %d 段, 全局动作上限 %d, 单段无进展上限 %d",
        ...
        while (step_counter < MAX_ONLINE_STEPS
                ...
                try:
                    ...
                    if pose_try == 0:
                        if rospy is not None:
                            rospy.sleep(0.5)
                        continue
                    rospy.logerr("[NavigationAgent] 在线决策时位姿不可用: %s", e)
            ...
            rospy.loginfo("[NavigationAgent] 在线决策 #%d: %s", online_calls, progress_view)
            try:
            ...
            if all_segs_done:
                rospy.loginfo("[NavigationAgent] 在线闭环结束: 全部 %d 段已核销", seg_count)
                break
            ...
            if correspond_seg > cursor:
                rospy.logwarn("[NavigationAgent] 决策试图跳到段%d（当前段%d），钳制为当前段",
            ...
            rospy.loginfo("[NavigationAgent] 执行动作 #%d（段%s, phase=%s）: %s.%s, params=%s",
            ...
            for attempt in range(SKILL_MAX_RETRIES + 1):
                if attempt > 0:
                    ...
                    rospy.logwarn("[NavigationAgent] 动作#%d 同参重试 %d/%d（上次: %s），等%.1fs",
                ...
                if result.get("success"):
                    rospy.loginfo("[NavigationAgent] 动作#%d 尝试 %d 成功",
            ...
            try:
                ...
                rospy.logwarn("[NavigationAgent] 动作后取位姿失败，有效性闸门跳过: %s", e)
            ...
            if result is not None:
                ...
                if not eff_ok:
                    ...
                    rospy.logwarn("[NavigationAgent] 动作#%d %s", step_counter, eff_note)
            ...
            if action_ok and want_done and correspond_seg == cursor:
                ...
                if can_complete:
                    ...
                    if gate_state == "met":
                        ...
                        n_gate_degraded += 1
                    rospy.loginfo("[NavigationAgent] 段%d 联合闸门通过(%s): %s",
                                  cursor, gate_state, gate_note)
                else:
                    n_gate_reject += 1
                    last_reject = gate_note
                    rospy.logwarn("[NavigationAgent] 段%d 声明完成但联合闸门驳回(%s): %s；继续补做该段",
                                  cursor, gate_state, gate_note)
            ...
            try:
                ...
                rospy.logwarn("[NavigationAgent] 增量任务快照保存失败(可忽略): %s", _e)
            ...
            if action_ok:
                ...
                if actual_seg_done:
                    ...
                    rospy.loginfo("[NavigationAgent] ✓ 段 %d 核销，已完成 %d/%d",
                                  cursor, len(completed_segs), seg_count)
                    cursor += 1
                    last_reject = ""
                rospy.loginfo("[NavigationAgent] 动作#%d 最终成功 (累计%.2fs)",
                              step_counter, elapsed_total)
            else:
                # 失败不独立 replan：下一轮在线 LLM 看到失败 + review 后自行换招/换参
                rospy.logwarn("[NavigationAgent] 动作#%d 失败: %s；交由下一轮在线决策换招",
            ...
            if (not actual_seg_done and cursor <= seg_count
                ...
                rospy.logerr("[NavigationAgent] %s", abort_reason)
                break
        else:
        ...
        rospy.loginfo("[NavigationAgent] 在线闭环汇总: success=%s, 段 %d/%d, 动作 %d 个, "
...
def main():
    parser = argparse.ArgumentParser(
        description="导航智能体全流程: 记忆检索 → 行动基准 → 在线逐步执行 → 写记忆 → Review",
