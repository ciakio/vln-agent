...
class MemoryManager:
    ...
    def __init__(self, memory_dir=None):
        ...
        logger.info("[Memory] 世界知识已加载: 物体 %d 个, 区域 %d 个, 历史任务 %d 条 (目录 %s)",
    ...
    def search(self, keywords):
        ...
        logger.info("[Memory] 记忆检索完成: 命中物体 %d 个, 历史任务 %d 条",
    ...
    def init_work_memory(self, task_id, instruction, plan=None):
        ...
        logger.info("[Memory] 初始化工作记忆: task_id=%s, 计划 %d 步", task_id, n_steps)
        self.work_memory = {
            "task_id": task_id,
    ...
    def add_step_result(self, step_idx, skill, action, params, result, elapsed_sec=0.0,
        ...
        logger.info("[Memory] 记录步骤 %d (%s.%s): %s, 耗时 %.2fs",
    ...
    def increment_replans(self):
        ...
        if self.work_memory is not None:
            ...
            logger.warning("[Memory] 重规划计数 +1, 当前 %d 次", self.work_memory["replans"])
    ...
    def update_semantic_map_realtime(self, perceptions):
        ...
        """
        if not PERSISTENT_MEMORY_ENABLED:
            logger.info("[Memory] 跨任务持久记忆已关闭, 跳过实时写库")
            return
        if not perceptions:
            return
        ...
        logger.info("[Memory] 实时写库检查: 物体 %d 个, 区域 %d 个 (置信度阈值 %.2f)",
        ...
        if changed:
            ...
            logger.info("[Memory] 实时写库完成, semantic_map 已落盘")
    ...
    def _merge_object(self, obj):
        ...
        if existing:
            # 更新已有物体
            logger.info("[Memory] 合并物体(更新已有): %s, 累计观测 %d 次",
            ...
            # 新物体
            logger.info("[Memory] 合并物体(新增): %s, 位置=%s, 置信度=%.2f",
    ...
    def save_snapshot(self):
        """保存工作记忆为完整任务快照 (不可变归档)。"""
        if self.work_memory is None:
            logger.warning("[Memory] 工作记忆为空, 跳过快照保存")
            return None
        ...
        logger.info("[Memory] 任务快照已归档: %s", path)
        return path
    ...
    def merge_to_persistent(self):
        ...
        """
        if not PERSISTENT_MEMORY_ENABLED:
            logger.info("[Memory] 跨任务持久记忆已关闭, 跳过合并到持久库")
            return
        if self.work_memory is None:
            return
        ...
        if changed:
            self.semantic_map["last_updated"] = _now()
            _save_json(self.semantic_map_file, self.semantic_map)
            logger.info("[Memory] 归档合并完成: 物体观测 %d 条, 区域 %d 条, 库内物体共 %d 个",
    ...
    def update_task_history(self, task_record):
        ...
        """
        if not PERSISTENT_MEMORY_ENABLED:
            logger.info("[Memory] 跨任务持久记忆已关闭, 跳过 task_history 写入")
            return
        record = {
        ...
        logger.info("[Memory] task_history 追加记录: %s (成功=%s, 耗时 %.1fs), 累计 %d 条",
