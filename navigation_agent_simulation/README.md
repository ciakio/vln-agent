...
```bash
navigation_agent_simulation/
├── simulation/                  # Habitat-Sim 仿真运行层（仿真专用）
│   ├── habitat_runtime.py       #   场景/虚拟相机/位姿/离散动作/阻塞寻路/轨迹录制
│   ├── habitat_runner.py        #   episode 加载与参考路径自检
│   ├── run_episode.py           #   环境自检入口（不经过智能体）
│   ├── run_agent.py             #   智能体主入口（单条 episode）
│   └── run_batch.py             #   批量入口（6 条 episode）
...
├── runs/                        # 仿真运行产物：episode_<id>_<ts>/、batch_summary.jsonl
├── logs/                        # 每次运行新建 run_<时间戳>/ 目录（见 2.1）
├── vln_agent_zsNo4HB9uLZ/       # 仿真数据集（6 条 episode）
├── zsNo4HB9uLZ/                 # MP3D 场景资产（glb/navmesh/house/semantic.ply）
└── skills/                      # 四大技能包（共 9 个 action）
    ├── __init__.py              # 技能注册表 + 统一 dispatch
    ├── common/                  # 跨 skill 公共库（不是第 5 个 skill）
    │   ├── sim_bridge.py        #   仿真运行时句柄注入（set/get_sim_runtime）
    │   ├── sensors.py          #   位姿读取 / 取图 / 深度 / 内参 / 坐标变换（仿真实现）
    │   ├── motion.py           #   离散运动控制（MotionController，0.25m/30°，仿真实现）
    │   ├── config.py           #   仿真相机/离散运动/模型/阈值配置
    │   ├── log.py              #   统一标准日志 logging
    ...
    └── execute_action/        # Skill 4: 基本动作 (2个子功能)
        ...
        └── move.py             #   沿指定方向前进指定距离（仿真内导航，不调 VLM）
```

智能体业务层（agent/planner/reviewer/memory/9 个 action）不直接接触仿真器，全部通过 `skills/common` 的冻结接口访问位姿/图像/运动；仿真运行时由 `simulation/run_agent.py` 启动后经 `sim_bridge.set_sim_runtime()` 注入一次。
...
仿真器只依赖 `habitat_sim`（conda 预编译包，无需克隆任何源码、不需要 habitat-lab），Ubuntu 20.04 适配：

```bash
conda create -n vln-sim python=3.10 -y && conda activate vln-sim
conda install -c aihabitat habitat-sim=0.3.3 headless -y
pip install numpy opencv-python openai
sudo apt install -y ffmpeg                                # 可选：轨迹视频转码
# 大模型密钥（文本 LLM 与 VLM）
...
数据集与场景已随工程携带，保持以下布局即可（名字与代码默认值一致，无需修改）：

```
项目根/
├── vln_agent_zsNo4HB9uLZ/dataset/val_unseen.json.gz   # 6 条 episode
└── zsNo4HB9uLZ/                                        # MP3D 场景 glb/navmesh/...
```

### 1.2 使用方式

#### 仿真环境自检（沿参考路径走一遍，不经过智能体，验证场景/导航网格/运动链路）

```bash
python -m simulation.run_episode --episode-id 505
```

#### 智能体自主编排（仿真主入口）

```bash
# 单条 episode（数据集自带指令；--record-video 可选，产出轨迹视频）
python -m simulation.run_agent --episode-id 505
python -m simulation.run_agent --episode-id 505 --record-video
# 自定义指令 / 模型 / 运动步数上限
python -m simulation.run_agent --episode-id 505 --instruction "找到白色桌子上的椰子水并靠近它"
python -m simulation.run_agent --episode-id 505 --max-steps 600
# 批量跑全部 6 条（505/506/507/1240/1241/1242），已完成的默认跳过，--overwrite 重跑
python -m simulation.run_batch
python -m simulation.run_batch --ids 505 506 --overwrite
```

运行产物落在 `runs/episode_<id>_<时间戳>/`：`console.log`（完整控制台）、`snapshot.json`
（任务结果与 success/SPL/path_length 等仿真指标）、`agent.log`、可选 `trajectory.mp4`/`telemetry.jsonl`；
批量汇总在 `runs/batch_summary.jsonl`。

```python
from simulation.habitat_runtime import HabitatRuntime
from skills.common.sim_bridge import set_sim_runtime
from agent import NavigationAgent
with HabitatRuntime(scenes_dir=".") as runtime:
    set_sim_runtime(runtime)
    runtime.reset(episode)                 # 复位到 episode 起点
...
#### 手动编排 Skill 序列（离线调试/回放，需先 set_sim_runtime 注入仿真运行时）

```
# 列出所有技能和功能
python main.py list
# 单步调用
python main.py call navigation navigate_by_route --target "黑色椅子" --angle 90
python main.py call vision_observe detect_object_360 --target "黑色椅子"
python main.py call close_to approach_diagonal --target "黑色椅子上的绿色饮料瓶" --scene-type shelf --direction 180
python main.py call navigation navigate_to_point --x 2.5 --y -3.4 --yaw 96
python main.py call execute_action turn --yaw 96
python main.py call execute_action move --distance 5
python main.py run-task --file auxiliary/examples/task_chair_water_customer.json
python main.py run-task --json '[{"skill":"execute_action","action":"move","params":{"distance":2}}]'
python main.py run-task --file steps.json --no-stop-on-failure
```
...
| --------------- | ---------------------------------------------------------- | ----- | ----------------------------------------------------------------- | -------------- |
| 在线自主闭环（默认、仿真主入口） | `agent.py` 的 `NavigationAgent`                             | 是     | 行动基准 + 每结束一个 action 做一次在线单步决策、段联合闸门核销（见 2.3）                      | 自然语言指令端到端自主执行  |
...
用户自然语言指令
    ...
    │     → run.log：本次运行的全部运行日志（[LLM#N] / [VLM.*#N] / 各 action 详细过程）
...
> 一句话记忆：
> **通道 A 用关键词翻两个持久文件取经验；可定位记忆视图 = 持久有坐标物体 (粗筛 / 回退全量)+ 本任务详细逐步流水，供单步直接填参。**
...
* **记录口径**：工作记忆与执行报告 `results` 中每个逻辑动作都**只记录最终一次结果**（重试过程在运行日志 run.log 中可追溯），发生过重试时该条附带 `retries` 字段，重试次数同时累计到全局 `skill_retries`。
...
| action              | 功能                                  | 适用场景                                              | 前置条件                   |
| ------------------- | ----------------------------------- | ------------------------------------------------- | ---------------------- |
| `navigate_to_point` | 导航到指定 (x,y,yaw) 固定坐标点，纯运动无感知        | 已知目标精确坐标                                          | 知道精确坐标和朝向，依赖导航网格 |
...
导航到指定位姿：向仿真内导航发送固定坐标点 (x,y,yaw) 并等待到达，纯运动、不看相机、不耗 VLM，依赖导航网格。
...
前进找物 + 到位旋转（步长自适应 + 0~2m 硬成功线）：**起步先看一帧**（目标已在 0~2m 就直接到位、不盲走）；尚未锁定目标时每步走 4m 大步搜索，一旦 VLM 锁定，就按**上一停车帧测得的目标深度**自适应缩短步长（≥6m 走 5m、4~6m 走 3m、2~4m 走 1m，越近越慢）；**只有目标深度落在 0~2m 成功线内才停止**（成功线硬固定、不可被传入参数覆盖），停稳后原地旋转到 `angle` 指定朝向 ——**找到参照物之后的转向由这一次 route 的 angle 一步完成、不另拆 turn**（转向归属见 2.3 第 8 点）。若某步运动执行实际没动（<0.1m），连续 2 次即判 `stalled` 失败交在线换招，且未推进帧不写目标全局坐标；末尾旋转没完全到位只告警、动作仍判成功，精确对正交后续 detect_object_360。
...
| action | 功能                        | 适用场景                                                | 前置条件                  |
| ------ | ------------------------- | --------------------------------------------------- | --------------------- |
| `turn` | 原地转到指定朝向，不平移、不看相机、不耗 VLM  | 只需转身 / 面向 / 掉头，或已知该转多少度                             | 无（只依赖运动执行 odom，不依赖导航网格） |
| `move` | 沿（当前或指定）朝向直线前进指定距离，不调 VLM | ①仅显式 "前进 N 米" 才用它开段；②段内无目标可逼近时的小段位置微调（progress、不核销） | 依赖导航网格；不支持后退    |

#### turn.py（原 execute_action 内的 rotate_to action）

原地转向：只旋转、x/y 位置不变，不调用相机、不做视觉搜索、不消耗 VLM，闭环完全基于 odom（复用 `common.MotionController.rotate_to` 的闭环控制），因此**不依赖导航网格，仿真内导航没加载网格也能转**。两种传参互斥（二选一，不能同时给、也不能都不给）：
...
```
python main.py call execute_action turn --yaw 90
python main.py call execute_action turn --delta-yaw -90
python3 -m skills.execute_action.turn --yaw 90     # 直接运行脚本
```

#### move.py（新增）

直线前进：用 odom 当前位姿 + 朝向 + distance 现算一个前方世界坐标，交仿真内导航 `motion.navigate_to` 移动并轮询 odom 到位，结束时 cancel + 到位截停；不调 VLM、会避障、依赖导航网格。`distance` 必填且 0 < distance ≤ 20；**不支持后退**（后退 = 先 `turn --delta-yaw 180` 掉头再 move）。可选 `--yaw / --delta-yaw`（互斥）先转向再走，都不给 = 沿当前朝向直走。

```
python main.py call execute_action move --distance 5
python main.py call execute_action move --distance 6 --delta-yaw 90   # 左转后前进6米
...
* `position{x,y,theta}` 是**全局 / 世界系坐标**，与位姿 odom、`navigate_to_point` 的目标点在同一坐标系：右手系，机器人正前方为 +x、左方为 +y；x/y 单位米，theta 单位度。它既不是相机像素，也不是相对机器人的偏移。
...
* 其中 `navigate_by_route` 在找到目标、深度落入 0~2m 成功线而停止的那一刻，用停车（最终旋转）前的机器人位姿 + VLM 偏移当场三角化出该物体的全局坐标并写入发现记录；若该步运动实际未推进（<0.1m），不据此帧写坐标，连续 2 次未推进直接判 stalled 失败。
...
| IROS  | IEEE/RSJ Intelligent Robots and Systems                   | 会议 | EI | ‑     | **C 类**   | IEEE 第二旗舰，实机落地首选阵地          |
