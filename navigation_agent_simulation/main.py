...
自主导航（LLM 理解任务 → 记忆检索 → 行动基准 → 在线逐步编排执行 → 联合核销 → Review）
请使用仿真入口 python -m simulation.run_agent（内部调用 agent.py 的 NavigationAgent）。

四大技能架构:
  1. navigation     : 前往目标附近 (navigate_to_point / navigate_by_route / navigate_by_goal)
  2. close_to       : 目标逼近 (approach_aligned / approach_diagonal)
  3. vision_observe : 查看实时视觉状态 (look_around / detect_object_360)
  4. execute_action : 基本动作 (turn / move)
...
==== Python API (智能体调用方式) ====
    ...
    result = agent.dispatch("navigation", "navigate_to_point", x=2.9, y=-3.8, yaw=0)
    result = agent.dispatch("execute_action", "turn", delta_yaw=90)
    result = agent.dispatch("execute_action", "move", distance=5)
...
==== 命令行方式 ====
    ...
    # 单步调用
    python3 main.py call vision_observe detect_object_360 --target "椅子"
    python3 main.py call close_to approach_aligned --target "椰子水" --scene-type shelf
    python3 main.py call navigation navigate_to_point --x 2.9 --y -3.8 --yaw 0
...
class RobotAgent:
    ...
    def __init__(self, run_name="robot_agent", stop_on_failure=True):
        """
        Args:
        ...
        logger.info("[RobotAgent] 导航智能体已初始化 (实例=%s, stop_on_failure=%s)",
                      run_name, stop_on_failure)
    ...
    def dispatch(self, skill, action, **params):
        ...
        Args:
            skill: 技能名称 (navigation / close_to / vision_observe / execute_action)
            action: 子功能名称
            **params: 子功能参数
        ...
        """
        logger.info("[RobotAgent] dispatch: skill=%s, action=%s, params=%s",
                      skill, action, params)
        try:
    ...
    def run_task_from_file(self, filepath):
        ...
        logger.info("[RobotAgent] 从文件加载任务: %s (%d 步)", filepath, len(steps))
...
def main():
    parser = argparse.ArgumentParser(
        description="机器人导航智能体主入口 — 四大技能统一调度",
