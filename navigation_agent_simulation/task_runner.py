...
class TaskRunner:
    ...
    def __init__(self, stop_on_failure=True):
        ...
        logger.info("[TaskRunner] 长序列任务执行器已初始化 (stop_on_failure=%s)",
                      stop_on_failure)
    ...
    def run(self, steps):
        ...
        logger.info("[TaskRunner] ===== 开始执行长序列任务 (%d 步) =====", len(steps))
        ...
        for idx, step in enumerate(steps):
            ...
            logger.info("[TaskRunner] --- 步骤 %d/%d: %s ---",
                          idx + 1, len(steps), step_name)
            logger.info("[TaskRunner]   skill=%s, action=%s, params=%s",
            ...
            if result.get("success"):
                logger.info("[TaskRunner]   ✓ 成功 (%.2fs): %s",
                              elapsed, result.get("message", ""))
                # 将当前步的 data 累积到上下文
                context.update(result.get("data", {}))
            else:
                logger.error("[TaskRunner]   ✗ 失败 (%.2fs): %s",
                             elapsed, result.get("message", ""))
                failed_step = idx + 1
                if self.stop_on_failure:
                    logger.error("[TaskRunner] 步骤 %d 失败，停止后续执行", idx + 1)
                    break
        ...
        logger.info("[TaskRunner] ===== 任务执行完毕: success=%s, 完成 %d/%d 步 =====",
