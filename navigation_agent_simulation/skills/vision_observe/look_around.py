...
if __package__:
    ...
    from ..common.sensors import capture_rgb, get_intrinsics, bgr_to_base64_uri
    ...
    from skills.common.sensors import capture_rgb, get_intrinsics, bgr_to_base64_uri
...
class Observer:
    ...
    def __init__(self, camera: str = "chest", out_dir: str = "observation_logs",
        ...
        logger.info("[observe] 初始化: camera=%s, 起始(%.3f,%.3f,%.1f°), 转回初始=%s, 输出=%s",
    ...
    def observe_8_directions(self):
        for angle in OBSERVE_ANGLES:
            logger.info("[observe] 旋转到 %d° (%s) ...", angle, DIRECTION_NAMES[angle])
            self.rotate_to(float(angle))
            self._sync_pose_from_odom()

            imgs = self.capture_view()
            if self.camera not in imgs:
                logger.error("[observe] %d° 图像采集失败", angle)
    ...
    def run(self):
        if not self.vlm.check_auth():
            logger.critical("VLM 鉴权失败")
            return {"success": False, "observations": [], "summary": "",
                    "memory_path": self.obs_path}
        try:
            logger.info("[observe] === 开始 8 方向环视 (%s) ===", self.camera)
            self._sync_pose_from_odom()
            self.initial_theta = self.theta
            self.observe_8_directions()
            if self.return_to_start:
                logger.info("[observe] 8 方向完成, 转回初始朝向 %.1f°", self.initial_theta)
                self.rotate_to(self.initial_theta)
                self._sync_pose_from_odom()
            self._flush()
            logger.info("[observe] 结果已落盘: %s, 累计语义物体 %d 个, 墙 %d 个",
            ...
            logger.info("[observe] 环视结束: success=%s, 有效方位 %d/%d",
...
def main():
    ...
    runner = Observer(camera=args.camera, out_dir=args.out_dir,
