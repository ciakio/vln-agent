...
ROTATE_NOOP_DEG = 0.2
# 补偿旋转后运动/相机稳定等待时间(秒), 原值 1.0 偏短易拍到运动模糊
SETTLE_SEC = 1.5
...
class RotateToTarget:
    ...
    def _analyze_current_view(self, intrinsics):
        ...
        if rgb_key not in images or depth_key not in images:
            logger.warning("[_analyze_current_view] 图像采集失败")
            return None
    ...
    def coarse_search(self, intrinsics):
        ...
        logger.info("[coarse_search] 基准朝向 base_theta=%.1f°, 各搜索方位为相对它的偏移角",
                      base_theta)

        for round_idx, angles in enumerate([COARSE_ANGLES_ROUND1, COARSE_ANGLES_ROUND2]):
            logger.info("[coarse_search] ===== 第 %d 轮粗搜索(相对基准): %s =====",
            ...
            for rel in angles:
                ...
                if result is not None and result.success:
                    logger.info("[coarse_search] 在相对 %d°(世界 %.1f°) 发现目标! depth=%.3fm",
                                  rel, world_angle, result.depth_target)
                    return True, result

        logger.warning("[coarse_search] 两轮粗搜索均未发现目标, 转回基准朝向 %.1f°", base_theta)
        # 没找到: 回到进入时的基准朝向
    ...
    def _verify_with_retry(self, intrinsics, retries=VERIFY_RETRY, tag="fine_align"):
        ...
        for attempt in range(retries):
            ...
            logger.warning("[%s] 第 %d 次验证未发现目标%s", tag, attempt + 1, detail)
        return None
    ...
    def fine_align(self, initial_result, intrinsics):
        """精对正: bbox 中心像素 + 内参计算角偏移, 闭环旋转到正对, 迭代验证。"""
        ...
        for iter_i in range(FINE_ALIGN_MAX_ITER):
            ...
            logger.info("[fine_align] iter=%d 像素偏移角=%.2f度 (容差 %.1f度)",
            ...
            if moved_deg < ROTATE_NOOP_DEG and abs(angle_offset_deg) <= RESIDUAL_ACCEPT_DEG:
                logger.info("[fine_align] iter=%d 底层旋转未动作(移动%.2f°)且残余偏角%.2f°<=%.1f°分辨率极限, 按已对正验证",
                              iter_i, moved_deg, angle_offset_deg, RESIDUAL_ACCEPT_DEG)
                verify = self._verify_with_retry(intrinsics)
                if verify is not None:
                    self.last_vlm_result = verify
                    return True
                logger.warning("[fine_align] iter=%d 死区放宽验证未过, 继续常规迭代", iter_i)

            # 等待位姿稳定后, 原地多帧验证(抗 VLM 单帧抖动), 仍失败再小步搜索
            ...
            if result is None or not result.success:
                ...
                if found_result is not None:
                    ...
                    logger.warning("[fine_align] iter=%d 补偿后验证与小步搜索均失败", iter_i)
                    return False

        logger.warning("[fine_align] 达到最大迭代次数 %d 仍未对正", FINE_ALIGN_MAX_ITER)
        return False
