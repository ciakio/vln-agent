...
=====================================
向仿真内导航发送目标坐标 (x, y, yaw) 并等待机器人到达，纯运动无感知。
实际目标发送复用 common.motion.send_navigation_goal（公共运动接口直发）。
...
class NavigateToPoint:
    ...
    def run(self):
        ...
        logger.info("[navigate_to_point] 开始导航到: x=%.3f, y=%.3f, yaw=%.1f°, task_type=%s",
        ...
        logger.info("[navigate_to_point] 目标已发送, 等待到达 (超时 %.0fs)...", self.NAV_TIMEOUT)
        t0 = time.time()
        arrived = False
        while True:
        ...
        if not arrived:
            logger.warning("[navigate_to_point] 导航超时(%.0fs), 残差 %.2fm, 执行截停",
            ...
            logger.info("[navigate_to_point] 到达, 最终位姿=%s, 耗时 %.1fs", final_pose, elapsed)
...
if __name__ == "__main__":
    ...
    runner = NavigateToPoint(args.x, args.y, args.z, args.yaw, args.task_type)
    out = runner.run()
    print(out)
