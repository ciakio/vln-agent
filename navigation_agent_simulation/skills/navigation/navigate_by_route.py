...
流程(步长自适应 + 0~2m 硬成功线):
  ...
  4. 停稳后原地闭环旋转到指定绝对角度; 旋转未完全到位仅告警、动作仍判成功,
...
class GoTurn:
    ...
    def __init__(self, target_description: str, final_angle_deg: float,
        ...
        logger.info("[GoTurn] 初始位姿: (%.3f, %.3f, %.1f°)", self.x, self.y, self.theta)
        logger.info("[GoTurn] 目标: %s", target_description)
        logger.info("[GoTurn] 前进方向: %.1f°, 未锁定搜索步长: %.1fm, 自适应分档 5/3/1m",
                      self.init_theta_deg, SEARCH_STEP)
        logger.info("[GoTurn] 成功线【硬固定】: 0<目标深度<%.1fm（传入 stop_depth=%.1fm 不放宽该线）",
                      SUCCESS_DEPTH_MAX, stop_depth)
        logger.info("[GoTurn] 最终旋转角度: %.1f°", final_angle_deg)
    ...
    def move_forward_step(self, distance=None):
        """沿初始方向前进 distance 米, 到达后停稳。返回 True 表示到达阈值附近。"""
        if distance is None:
        ...
        logger.info("[move_forward_step] === 前进 %.1fm ===", distance)
        logger.info("[move_forward_step] 起点: (%.3f, %.3f), 目标: (%.3f, %.3f)",
        ...
        while True:
            ...
            if time.time() - t0 > MOVE_TIMEOUT:
                logger.warning("[move_forward_step] 超时 (%.1fs), 当前距离=%.3fm", MOVE_TIMEOUT, dist)
                break
            time.sleep(0.1)

        # 覆盖目标强行截停底层导航
        try:
            logger.info("[move_forward_step] 发送原地位姿覆盖目标, 强行截停...")
            self.motion.cancel_navigation()
        except Exception as e:
            logger.warning("[move_forward_step] 覆盖目标失败 (可忽略): %s", str(e))

        # 强制停稳 + 物理缓冲
        logger.info("[move_forward_step] 执行强制停稳, 等待位姿稳定...")
        ...
        logger.info("[move_forward_step] 移动完成: pos=(%.3f, %.3f), 实际前进=%.3fm, arrived=%s",
    ...
    def run(self):
        logger.info("=" * 60)
        logger.info("[GoTurn] 开始前进找物 + 到位旋转")
        logger.info("[GoTurn] 目标: %s", self.target_description)
        logger.info("=" * 60)
        ...
        logger.info("=" * 60)
        logger.info("[GoTurn] 起步先看一帧, 判断目标是否已在成功距离 (0,%.1fm) 内", SUCCESS_DEPTH_MAX)
        logger.info("=" * 60)
        try:
            ...
            if rgb_key in images0 and depth_key in images0:
                ...
                if r0.success and 0.0 < r0.depth_target:
                    ...
                    if r0.depth_target < SUCCESS_DEPTH_MAX:
                        ...
                        logger.info("[GoTurn] 起步帧即见目标: depth=%.3fm 在成功线(<%.1fm)内, 直接到位, 不前进",
                                      _fwd0, SUCCESS_DEPTH_MAX)
                        found_target = True
                    else:
                        logger.info("[GoTurn] 起步帧锁定目标 d=%.3fm(>=%.1fm), 按自适应分档逼近",
                                      last_depth, SUCCESS_DEPTH_MAX)
                else:
                    logger.info("[GoTurn] 起步帧未见目标, 以 %.1fm 大步开始搜索", SEARCH_STEP)
            else:
                logger.warning("[GoTurn] 起步帧采集失败, 按未锁定目标大步搜索")
        except Exception as e:
            logger.warning("[GoTurn] 起步先看异常(%s), 按未锁定目标大步搜索", e)
        ...
        while total_dist < self.max_dist and not found_target:
            ...
            if reached:
                logger.info("[GoTurn] 目标已在成功线内, 停止前进")
                found_target = True
                break
            if step < 0.05:
                logger.warning("[GoTurn] 剩余距离 %.3fm 过小, 停止前进", remaining)
                break

            _ddesc = "未锁定" if last_depth is None else "%.2fm" % last_depth
            logger.info("[GoTurn] ===== 第 %d 次「前进+停车搜索」: 上一帧深度=%s, 本段 %.2fm, 累计将达 %.2f/%.1fm =====",
            ...
            logger.info("[GoTurn] 累计前进: %.2fm / %.1fm", total_dist, self.max_dist)

            # 未推进保护：实际几乎没动 → 不据此帧判成功、不写目标坐标；连续 STALL_LIMIT 次判失败
            if actual_dist < MIN_ADVANCE:
                stall_count += 1
                logger.warning("[GoTurn] 本步实际前进 %.3fm < %.2fm（未推进 %d/%d）, 不据此帧判成功、不写目标坐标",
                              actual_dist, MIN_ADVANCE, stall_count, STALL_LIMIT)
                if stall_count >= STALL_LIMIT:
                    self.status = "stalled"
                    self.message = f"连续 {STALL_LIMIT} 次前进未推进(运动执行异常), 终止并交在线换招"
                    logger.error("[GoTurn] %s", self.message)
                    return False
                continue
            stall_count = 0

            # 采集图像 + VLM 分析
            logger.info("[GoTurn] 第 %d 次停车: 采集图像并调用 VLM ...", search_round)
            try:
                images = capture_rgb_depth(self.camera)
            except Exception as e:
                logger.warning("[GoTurn] 图像采集异常(%s), 继续下一轮", e)
                continue
            if rgb_key not in images or depth_key not in images:
                logger.warning("[GoTurn] 图像采集失败, 继续前进")
                continue
            ...
            if result.success and 0.0 < result.depth_target:
                logger.info("[GoTurn] 找到目标! depth_target=%.3fm", result.depth_target)
                if result.depth_target < SUCCESS_DEPTH_MAX:
                    _fwd = self._mark_found(result)
                    logger.info("[GoTurn] 目标深度 %.3fm 在成功线 (0,%.1fm) 内, 停止前进",
                                  _fwd, SUCCESS_DEPTH_MAX)
                    logger.info("[GoTurn] 触发停止目标物体全局位姿: (%.3f, %.3f), 前向%.2fm 右偏%.3fm",
                ...
                logger.info("[GoTurn] 目标深度 %.3fm >= %.1fm, 缩短步长继续逼近",
                              last_depth, SUCCESS_DEPTH_MAX)
            else:
                logger.info("[GoTurn] 未找到目标: %s", result.message)

        if not found_target:
            logger.error("[GoTurn] 前进 %.1fm 未找到成功线 (0,%.1fm) 内的目标, 任务失败",
        ...
        logger.info("[GoTurn] 开始旋转到最终角度: %.1f°", self.final_angle_deg)
        ...
        if not rotate_ok:
            ...
            logger.warning("[GoTurn] 末尾旋转未完全到位(残差≈%.1f°); 目标已在 (0,%.1fm) 找到, "
                          "动作仍判成功, 精确对正交后续 detect_object_360",
                          self.align_residual, SUCCESS_DEPTH_MAX)

        self.status = "arrived"
        logger.info("=" * 60)
        logger.info("[GoTurn] 完成! 最终位姿: (%.3f, %.3f, %.1f°)", self.x, self.y, self.theta)
        logger.info("=" * 60)
        return True
