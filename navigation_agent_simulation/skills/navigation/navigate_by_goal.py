...
if __package__:
    ...
    from ..common.sensors import capture_rgb, bgr_to_base64_uri
    ...
    from skills.common.sensors import capture_rgb, bgr_to_base64_uri
...
class Explorer:
    ...
    def __init__(self, target_name="白色桌子上的白绿色的椰子水饮料瓶", out_dir="explore_logs",
        ...
        logger.info("[explore] 初始化: target=%s, max_rounds=%d, camera=%s, 起始位姿(%.3f,%.3f,%.1f°)",
                      self.target_name, self.max_rounds, self.camera, self.x, self.y, self.theta)
    ...
    def move_to(self, target_x, target_y):
        """导航前进到绝对目标位置，到达后取消导航并强制停稳。"""
        self._sync_pose_from_odom()
        start_x, start_y = self.x, self.y
        initial_dist = math.hypot(target_x - start_x, target_y - start_y)
        logger.info("[explore.move_to] 目标(%.3f,%.3f), 初始距离 %.3fm",
                      target_x, target_y, initial_dist)
        ...
        try:
            ...
            logger.warning("[move_to] 取消导航失败 (可忽略): %s", str(e))
        ...
        ok = final_dist < ARRIVE_TOLERANCE
        logger.info("[explore.move_to] 结束: 剩余 %.3fm, 到达=%s, 实际位姿(%.3f,%.3f)",
                      final_dist, ok, self.x, self.y)
        return ok
    ...
    def scan_round(self, round_i, angles_to_scan):
        ...
        def vlm_worker(view_i, theta_deg, robot_x, robot_y, img, img_paths):
            ...
            _scene = analysis.get("scene", {}) or {}
            logger.info("[explore.scan] r%d 方位 %.1f°: found=%s, conf=%s, open=%.2f, 物体%d个, %s",
                          round_i, theta_deg, analysis.get("found"),
                          analysis.get("confidence"),
                          _safe_float(analysis.get("open_area_score")),
                          len(_scene.get("objects", []) or []),
                          analysis.get("environment_desc", ""))
            entry = {
            ...
            with lock:
                ...
                if analysis.get("found") and found_data["entry"] is None:
                    logger.info("[explore.scan] *** 在 %.1f° 方位发现目标! 位姿(%.3f,%.3f) ***",
                                  theta_deg, robot_x, robot_y)
        ...
        for view_i, target_theta in enumerate(angles_to_scan):
            ...
            self.view_i = view_i
            logger.info("[explore.scan] r%d 扫描方位 %d/%d -> %.0f°",
                          round_i, view_i + 1, len(angles_to_scan), target_theta)
            ...
            if self.camera not in imgs:
                logger.warning("[explore.scan] %.0f° 图像采集失败, 记为 None", target_theta)
                results.append(None)
                continue
        ...
        if found_event.is_set() and found_data["pose"] is not None:
            ...
            logger.info("[explore.scan] 本轮发现目标, 旋转回发现方位 %.1f°", theta_found)
            if not self.rotate_to(theta_found):
                logger.warning("[scan_round] 旋转回发现方位 %.1f° 未到位", theta_found)
            self._sync_pose_from_odom()
            return "found"
        return results
    ...
    def choose_direction(self, results):
        ...
        for i, r in enumerate(results):
            ...
            logger.info("[choose_direction] 候选 %d: theta=%.1f°, open=%.2f, novel=%.0f, score=%.2f",
                          len(cands), theta, open_score, novelty, score)

        if not cands:
            logger.warning("[choose_direction] 无可用候选方向")
            return None
        cands.sort(key=lambda c: c[0], reverse=True)
        for c in cands[:3]:
            logger.info("[choose_direction] top: theta=%.1f°, score=%.2f (open=%.2f, novel=%.0f)",
                          c[1], c[0], c[4], c[5])
        logger.info("[choose_direction] 选择: theta=%.1f°, score=%.2f", cands[0][1], cands[0][0])
    ...
    def run(self):
        try:
            ...
            while round_i < self.max_rounds:
                logger.info("[explore] ===== 第 %d/%d 轮探索 =====",
                              round_i + 1, self.max_rounds)
                # P1 #17: 第一轮初始对齐到全局 0°
                if round_i == 0:
                    self.rotate_to(0.0)

                angles_to_scan = self.get_angles_to_scan(round_i, forward_theta)
                logger.info("[explore] 本轮扫描方位: %s", angles_to_scan)
                ...
                if not valid:
                    fail_rounds += 1
                    logger.warning("[explore] 本轮无有效采集 (连续 %d/%d 轮)",
                                  fail_rounds, MAX_CAPTURE_FAIL_ROUNDS)
                ...
                if chosen is None:
                    logger.warning("[explore] 无可用开阔方向, 尝试回退到上一 waypoint")
                    ...
                    logger.info("[explore] 沿选定方向 %.1f° 前进 %.1fm (score=%.2f)",
                                  theta_best, STEP_FORWARD_DIST, score_best)
    ...
    def _finalize(self, status):
        self.status = status
        logger.info("[explore] 探索结束: status=%s, 共 %d 条视野记忆, %d 个语义物体, waypoint %d 个",
                      status, len(self.memory), len(self.semantic_objects), len(self.waypoints))
        self._flush_memory()
        self._flush_grid()
        self._flush_semantic_map()
