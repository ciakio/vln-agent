...
流程:
  ...
  4. 沿指定方向导航前进, 到位后强制停稳
  5. 闭环原地旋转到最终朝向 (direction ± 90°, 仍是 0/90/180/270 之一)
"""
...
if __package__:
    ...
    from ..common.sensors import capture_rgb_depth, get_intrinsics, normalize_angle
    from ..common.vlm import VLMApproach
else:
    import os as _os
    sys.path.insert(0, _os.path.abspath(_os.path.join(_os.path.dirname(__file__), '..', '..')))
    from skills.common.motion import MotionController
    from skills.common.sensors import capture_rgb_depth, get_intrinsics, normalize_angle
...
class ApproachDiagonal:
    ...
    def move_along_direction(self, direction_deg, distance):
        ...
        while True:
            ...
            if time.time() - t0 > MOVE_TIMEOUT:
                logger.warning("[move_along_direction] 超时 (%.1fs), 当前距离=%.3fm",
                              MOVE_TIMEOUT, dist)
                break
            time.sleep(0.1)
        ...
        try:
            ...
            logger.warning("[move_along_direction] 覆盖目标失败 (可忽略): %s", str(e))

        # 强制停稳 + 物理缓冲（与原始验证代码时序一致：停稳各 0.2s + 1.0s）
    ...
    def run(self) -> bool:
        logger.info("[DiagonalApproach] 开始斜角度逼近, 目标: %s", self.target_description)
        ...
        if rgb_key not in images or depth_key not in images:
            logger.error("[DiagonalApproach] 图像采集失败")
        ...
        if not result.success:
            logger.error("[DiagonalApproach] VLM 未找到目标: %s", result.message)
            self.status = "not_found"
            return False

        D = result.depth_target
        self.last_depth_m = D
        logger.info("[DiagonalApproach] 找到目标! 直线距离 D=%.3fm", D)
        ...
        if abs(angle_diff_deg) > 90.0:
            logger.error("[DiagonalApproach] 角度差 %.2f° 超出 ±90° 范围", angle_diff_deg)
        ...
        logger.info("[DiagonalApproach] 几何解算: 当前朝向=%.1f°, 前进方向=%.1f°, 夹角=%.2f°, "
                      "前进距离=D·cos=%.3fm, 转身后剩余=D·|sin|=%.3fm, 转向=%s, 最终朝向=%.1f°",
                      cur_theta_deg, self.direction_deg, angle_diff_deg,
                      forward_dist, self.remaining_distance, turn_dir, final_heading)
        ...
        if forward_dist >= MIN_MOVE_DIST:
            ...
            if not move_ok:
                logger.warning("[DiagonalApproach] 前进未在阈值内到达, 继续尝试旋转对正")
        else:
            logger.info("[DiagonalApproach] 前进距离 %.3fm < %.1fm, 跳过移动",
                          forward_dist, MIN_MOVE_DIST)

        # 原地旋转到最终朝向
        rotate_ok = True
        if turn_dir != "不转":
            logger.info("[DiagonalApproach] 开始%s到最终朝向: %.1f°", turn_dir, final_heading)
        ...
        if not move_ok or not rotate_ok:
            ...
            logger.error("[DiagonalApproach] 逼近未完全成功 (move=%s, rotate=%s)",
                         move_ok, rotate_ok)
            return False

        self.status = "arrived"
        logger.info("[DiagonalApproach] 完成! 最终位姿: (%.3f, %.3f, %.1f°)",
