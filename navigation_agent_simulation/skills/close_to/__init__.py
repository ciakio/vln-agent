...
ACTION_DEFS = {
    "approach_aligned": {
        "description": "视野中已存在目标且已正对时的逼近：拍照+VLM定位目标，计算左右偏移和前后距离，"
                       "导航到目标附近（距支撑面0.5m）。"
                       "适用场景: detect_object_360 之后，目标已在视野中央。"
                       "前置条件: 目标在视野中且已正对（通常 detect_object_360 之后）。",
        "params": {
...
def _exec_approach_aligned(p):
    ...
    if success:
        ...
        if r is not None:
            ...
            # approach_aligned 内部已用像素偏移算出物体全局 target_point，直接收录为坐标
            if node.target_point is not None:
                aligned_entry["position"] = {
                    "x": round(node.target_point[0], 3),
                    "y": round(node.target_point[1], 3),
                }
            perceptions["objects_found"].append(aligned_entry)
        if node.target_point is not None:
...
def _exec_approach_diagonal(p):
    ...
    if success:
        ...
        if r is not None:
            ...
            # 转 90° 对正后机器人正对目标，剩余距离沿当前朝向投影即物体全局坐标
            gx, gy = project_to_global(getattr(node, "x", 0.0), getattr(node, "y", 0.0),
                                       getattr(node, "theta", 0.0), depth_for_memory)
            if gx is not None:
                diag_entry["position"] = {"x": gx, "y": gy}
            perceptions["objects_found"].append(diag_entry)
...
def run(action, **params):
    ...
    logger.info("[close_to] 执行 action=%s, params=%s", action, validated)
    try:
        result = executor(validated)
        logger.info("[close_to] action=%s 结束: success=%s, message=%s",
